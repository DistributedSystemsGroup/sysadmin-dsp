...
log = logging.getLogger("dnsupdate")
...
def verifymyInput(myInput):
    ...
    if action == 'delete' or action == 'del': # skip type checks
        return 'del', type   # Bail out early
...
server = "127.0.0.1"
keyfile = "/mnt/cephfs/admin/rndc.key"
origin = None
doPTR = True
TTL = "300"
domain = ".vms.bigfoot.eurecom.fr."
...
class DDNSUpdate:
    ...
    def update(self, hostname, ip, instance_id):
        input = ["add", str(hostname + domain), "A", str(ip)]
        print("DDNS record: '%s'" % str(input))
        doUpdate(server, keyfile, origin, TTL, doPTR, input)

    def remove(self, hostname, ip, instance_id):
        input = ["del", str(hostname + domain), "A", str(ip)]
        print("DDNS record: '%s'" % str(input))
        doUpdate(server, keyfile, origin, TTL, doPTR, input)
...
#    doUpdate(myArgs.Server, myArgs.Key, myArgs.Origin, myArgs.TimeToLive, myArgs.doPTR, myArgs.myInput)
    doUpdate(server, keyfile, origin, TTL, doPTR, sys.argv[1:])

if __name__ == "__main__":
    main()
#    del_host("test", "192.168.45.25")
